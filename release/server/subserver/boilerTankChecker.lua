local var = {
    tankLeft = peripheral.wrap("left"),
    tankRight = peripheral.wrap("right"),
    tankReserve = peripheral.wrap("front"),
    waterLeft = true,
    waterRight = true,
    waterFront = true
}
while true do
    ...
    local tankL = #fluidLeft > 0 and fluidLeft[1].amount > 0
    local tankR = #fluidRight > 0 and fluidRight[1].amount > 0
    local tankF = #fluidReserve > 0 and fluidReserve[1].amount > 0
    ...
    var.waterLeft = tankL
    var.waterRight = tankR
    var.waterFront = tankF
end